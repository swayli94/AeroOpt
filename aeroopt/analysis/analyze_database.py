...
from aeroopt.core.individual import Individual, SORT_BY_CROWDING
...
class AnalyzeDatabase:
    ...
    def update_data_arrays(self) -> None:
        ...
        for i in range(nn):
            indi = self.database.individuals[i]
            self._xs[i,:] = indi.x
            # A failed evaluation leaves `y` empty; keep the row at zero instead
            # of raising a broadcast error, so a total database that contains
            # failed runs can still be analysed.
    ...
    def calculate_typical_distance(self,
        ...
        # Tell the database its crowding distances are current, otherwise
        # `sort_database(sort_type=6)` rejects the sort.
        self.database._updated_crowding_distance = True
    ...
    def calculate_potential_induced_by_database(self,
        ...
        # `_coef_potential` starts at 0.0, which would make `func_potential`
        # return 1 for every distance; derive it from the database first.
        if self._coef_potential <= 0.0:
    ...
    def eliminate_crowding_individuals(self,
        ...
        self.database.sort_database(sort_type=SORT_BY_CROWDING)
        ...
        while len(ID_list_eliminated) < n_max_delete and self.size > n_min_left:
            ...
            # Crowding metrics need at least 2 individuals; with fewer, both the
            # recomputation and the crowding sort are meaningless (and the sort
            # would raise because the deletion cleared the "updated" flag).
            if self.size <= 1:
                break
    ...
    def calculate_grouping(self,
        ...
        random_state: int|None
            Seed for the k-means centroid initialization. The default makes the
            grouping reproducible; pass None for the scikit-learn default.
        '''
        if update_attributes:
            self.update_attributes()

        kmeans = KMeans(n_clusters=n_groups, random_state=random_state).fit(self._scaled_variables)
