...
SAO_INNER_MAX_ITERATIONS = 10


def sao_inner_de_rng_seed(bench_index: int) -> int:
    return MASTER_SEED + bench_index * 1_000_000 + 70_000_001


def sao_outer_de_rng_seed(bench_index: int) -> int:
    return MASTER_SEED + bench_index * 1_000_000 + 90_000_001
...
def build_settings_file(
    ...
    settings = {
        ...
        "zdt_sao_problem": {
            ...
            "name": "zdt_sao_problem",
            "name_data_settings": "zdt_sao_data",
            "output_type": [-1, -1],
        ...
        "zdt_sao_inner_opt": {
            "type": "SettingsOptimization",
            "name": "zdt_sao_inner_opt",
            "resume": False,
            "population_size": SAO_INNER_POPULATION_SIZE,
            "max_iterations": SAO_INNER_MAX_ITERATIONS,
...
def plot_subplot(ax, opt: SAO, title: str, vmax_gen: int,
...
def run_one_benchmark(
    ...
    data = SettingsData("zdt_sao_data", fname_settings=str(settings_path))
    problem = Problem(
        data,
        SettingsProblem("zdt_sao_problem", data, fname_settings=str(settings_path)),
    )
    opt_settings = SettingsOptimization("zdt_sao_opt", fname_settings=str(settings_path))
    opt_settings.working_directory = str(work_dir)

    inner_opt_settings = SettingsOptimization("zdt_sao_inner_opt", fname_settings=str(settings_path))
    inner_opt_settings.working_directory = str(inner_work_dir)

    alg_settings = SettingsDE("zdt_sao_alg", fname_settings=str(settings_path))
    ...
    opt_inner = OptDE(
        ...
        mp_evaluation=None,
        rng=np.random.default_rng(int(sao_inner_de_rng_seed(bench_index))),
        logging=False,
    )
    ...
    sao.post_process = PostProcessSAO(sao, surrogate)
...
def main() -> None:
    ...
    for i, (ax, (bname, bfn)) in enumerate(zip(axes, BENCHMARKS)):
        print(f"SAO example: running {bname} ...", flush=True)
        subdir = run_root / bname
        opt_sao = run_one_benchmark(bfn, subdir, mp_eval, bench_index=i, benchmark_name=bname)
        plot_subplot(ax, opt_sao, bname, MAX_ITERATIONS, show_pareto_label=True)
    ...
    out_png = script_dir / "sao_zdt_subplots.png"
