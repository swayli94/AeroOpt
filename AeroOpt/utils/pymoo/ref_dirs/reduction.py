...
from AeroOpt.utils.pymoo.misc import cdist

from AeroOpt.utils.pymoo.ref_dirs.misc import project_onto_unit_simplex_recursive
from AeroOpt.utils.pymoo.reference_direction import ReferenceDirectionFactory, sample_on_unit_simplex, \
...
def kmeans(X, centroids, n_max_iter, a_tol, n_ignore):
    """Lloyd-style k-means on ``X`` with simplex projection; only updates centroids with index >= ``n_ignore``."""
...
class ReductionBasedReferenceDirectionFactory(ReferenceDirectionFactory):
    """Subselects ``n_points`` well-spread directions from dense simplex samples plus edge lattice, optional k-means polish."""
    ...
    def __init__(self,
                 ...
                 kmeans_a_tol=0.0001,
                 **kwargs):
        """Set dimension, target count, dense sampling size/method, and k-means refinement parameters."""
    ...
    def _do(self, random_state=None):
        """Fuse uniform lattice edges with random simplex points, pick max-distance seeds, optionally run constrained k-means."""
