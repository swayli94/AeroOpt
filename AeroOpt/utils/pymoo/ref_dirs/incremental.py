...
from AeroOpt.utils.pymoo.reference_direction import ReferenceDirectionFactory


def check_n_points(n_points, n_dim):
    """Find ``n_partitions`` whose incremental lattice has exactly ``n_points`` rows, or return bracketing sizes for errors."""
...
def incremental_lattice(n_partitions, n_dim):
    """Build the uniform simplex lattice by repeatedly expanding the outer shell; return normalized weight rows."""
    I = n_dim * np.eye(n_dim)
    W = np.zeros((1, n_dim))
    edgeW = W
...
class IncrementalReferenceDirectionFactory(ReferenceDirectionFactory):
    """Reference directions from the incremental simplex lattice; resolves ``n_partitions`` from ``n_points`` when given."""

    def __init__(self, n_dim, scaling=None, n_points=None, n_partitions=None, **kwargs) -> None:
        """Require either ``n_partitions`` or a compatible ``n_points``; raises if no uniform lattice exists at that count."""
    ...
    def _do(self, **kwargs):
        """Generate and return the incremental lattice for ``self.n_partitions`` and ``self.n_dim``."""
